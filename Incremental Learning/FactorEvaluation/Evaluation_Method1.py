### Evaluate the performance of the factors, such as IC, CAP_IC, and Group_IC for Method1
...
def calPerformance(predictY, testY, df_cap, num_cap=5, num_group=10):
        ...
        for date, group in df_cap.groupby(level='tradingDate'):
            ...
            # sub_ds_cap_divide = ds_cap_divide[ds_cap_divide.index.get_level_values('tradingDate') == date]
...
# df2 = b[(b.index.get_level_values(0) >= 20210104) & (b.index.get_level_values(0) <= 20211222)]
# df2 = df2.reset_index()
...
testIC, corr, mean, std, testICIR, ic_cap_group_mean , group_alpha, group_alpha_mean =calPerformance(predictY, testY, df_cap, num_cap=5, num_group=10)
# pdb.set_trace()
